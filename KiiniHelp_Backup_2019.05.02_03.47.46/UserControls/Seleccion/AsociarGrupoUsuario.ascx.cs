...
namespace KiiniHelp.UserControls.Seleccion
{
    public partial class AsociarGrupoUsuario : UserControl
    {
        ...
        public bool EspecialConsulta
        {
            ...
            set
            { HabilitaGrupos((int)BusinessVariables.EnumRoles.AccesoAnalíticos, value); }
        }
        ...
        public bool ValidaCapturaGrupos()
        {
            ...
            try
            {
                ...
                if (EspecialConsulta)
                    if (GruposAsociados.Cast<RepeaterItem>().Select(item => (Label)item.FindControl("lblIdTipoSubGrupo")).Count(lblIdRol => int.Parse(lblIdRol.Text) == (int)BusinessVariables.EnumRoles.AccesoAnalíticos) <= 0)
