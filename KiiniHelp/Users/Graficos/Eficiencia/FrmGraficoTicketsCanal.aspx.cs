    ...
    public partial class FrmGraficoTicketsCanal : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ucFiltrosTicketCanal.ObtenerParametros();
                    GeneraGraficas();
                }
            }
            catch (Exception ex)
        ...
        private void GeneraGraficas()
        {
            try
            {
                ...
                GeneraGraficaPareto(rhcTicketsPareto, dtDatos, "Canal");
        ...
        private void GeneraGraficaPareto(RadHtmlChart grafico, DataTable dt, string stack)
        {
            try
            {
                ...
                switch (ucFiltrosTicketCanal.TipoPeriodo)
                {
                    case 1:
                        lblPiePareto.Text = string.Format("Diario: {0} - {1}",
                            ucFiltrosTicketCanal.FiltroFechas == null ? "Historico" : ucFiltrosTicketCanal.FiltroFechas.Single(s => s.Key == "inicio").Value.ToShortDateString(),
                            ucFiltrosTicketCanal.FiltroFechas == null ? string.Empty : ucFiltrosTicketCanal.FiltroFechas.Single(s => s.Key == "fin").Value.ToShortDateString());
                        break;
                    case 2:
                    ...
                    case 3:
                        lblPiePareto.Text = string.Format("Mensual: {0} - {1}",
                            ucFiltrosTicketCanal.FiltroFechas == null ? "Historico" : ucFiltrosTicketCanal.FiltroFechas.Single(s => s.Key == "inicio").Value.ToString("MMM"),
                            ucFiltrosTicketCanal.FiltroFechas == null ? string.Empty : ucFiltrosTicketCanal.FiltroFechas.Single(s => s.Key == "fin").Value.ToString("MMM"));
                        break;
                    case 4:
