    ...
    public partial class FrmGraficoTicketsOrganizacion : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ucFiltrosTicketOrganizacion.ObtenerParametros();
                    GeneraGraficas();
                }
            }
            catch (Exception ex)
        ...
        private void GeneraGraficas()
        {
            try
            {
                ...
                if (topResult.Count > 20)
                {
                    ...
                    {
                        DataRow dr = dtResultTop.Select("Id=-1").FirstOrDefault(); // finds all rows with id==2 and selects first or null if haven't found any
                        if (dr != null)
                        {
                            //dr[i] = dtDatos.Compute("Sum(" + dtDatos.Columns[i] + ")", "");
                ...
                GeneraGraficaPareto(rhcTicketsPareto, dtResultTop, "Organizaciones");
        ...
        private void GeneraGraficaPareto(RadHtmlChart grafico, DataTable dt, string stack)
        {
            try
            {
                ...
                switch (ucFiltrosTicketOrganizacion.TipoPeriodo)
                {
                    case 1:
                        lblPiePareto.Text = string.Format("Diario: {0} - {1}",
                            ucFiltrosTicketOrganizacion.FiltroFechas == null ? "Historico" : ucFiltrosTicketOrganizacion.FiltroFechas.Single(s => s.Key == "inicio").Value.ToShortDateString(),
                            ucFiltrosTicketOrganizacion.FiltroFechas == null ? string.Empty : ucFiltrosTicketOrganizacion.FiltroFechas.Single(s => s.Key == "fin").Value.ToShortDateString());
                        break;
                    case 2:
                    ...
                    case 3:
                        lblPiePareto.Text = string.Format("Mensual: {0} - {1}",
                            ucFiltrosTicketOrganizacion.FiltroFechas == null ? "Historico" : ucFiltrosTicketOrganizacion.FiltroFechas.Single(s => s.Key == "inicio").Value.ToString("MMM"),
                            ucFiltrosTicketOrganizacion.FiltroFechas == null ? string.Empty : ucFiltrosTicketOrganizacion.FiltroFechas.Single(s => s.Key == "fin").Value.ToString("MMM"));
                        break;
                    case 4:
