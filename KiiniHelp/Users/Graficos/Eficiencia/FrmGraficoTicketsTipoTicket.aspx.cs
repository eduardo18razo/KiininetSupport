    ...
    public partial class FrmGraficoTicketsTipoTicket : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ucFiltrosTicketTipoTicket.ObtenerParametros();
                    GeneraGraficas();
                }
            }
            catch (Exception ex)
        ...
        private void GeneraGraficas()
        {
            try
            {
                ...
                GeneraGraficaPareto(rhcTicketsPareto, dtDatos, "Tipo Ticket");
        ...
        private void GeneraGraficaPareto(RadHtmlChart grafico, DataTable dt, string stack)
        {
            try
            {
                switch (ucFiltrosTicketTipoTicket.TipoPeriodo)
                {
                    case 1:
                        grafico.ChartTitle.Text = string.Format("Pareto diario: {0} - {1}",
                            ucFiltrosTicketTipoTicket.FiltroFechas == null ? "Historico" : ucFiltrosTicketTipoTicket.FiltroFechas.Single(s => s.Key == "inicio").Value.ToShortDateString(),
                            ucFiltrosTicketTipoTicket.FiltroFechas == null ? string.Empty : ucFiltrosTicketTipoTicket.FiltroFechas.Single(s => s.Key == "fin").Value.ToShortDateString());
                        break;
                    case 2:
                    ...
                    case 3:
                        grafico.ChartTitle.Text = string.Format("Pareto mensual: {0} - {1}",
                            ucFiltrosTicketTipoTicket.FiltroFechas == null ? "Historico" : ucFiltrosTicketTipoTicket.FiltroFechas.Single(s => s.Key == "inicio").Value.ToString("MMM"),
                            ucFiltrosTicketTipoTicket.FiltroFechas == null ? string.Empty : ucFiltrosTicketTipoTicket.FiltroFechas.Single(s => s.Key == "fin").Value.ToString("MMM"));
                        break;
                    case 4:
